...
export function listComplaints() {
  ...
  return db.prepare('SELECT * FROM complaints ORDER BY created_at DESC').all() as ComplaintRow[];
}

export function getComplaintById(id: number): ComplaintRow | null {
  const db = getDatabase();
  const row = db.prepare('SELECT * FROM complaints WHERE id = ?').get(id) as ComplaintRow | null;
  return row || null;
}
