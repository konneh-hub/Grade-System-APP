...
export async function POST(req: Request) {
  ...
  if (existing.status !== 'published') return NextResponse.json({ error: 'Only published results can be unpublished' }, { status: 409 });

  db.prepare('UPDATE results SET status = ? WHERE id = ?').run('approved', resultId);
  writeAudit('Result unpublished', resultId, { unpublished_by: guard.auth.user.id, role: guard.auth.roles.join(',') });
