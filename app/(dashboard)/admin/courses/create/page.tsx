  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={onSubmit} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        <label className="text-sm">Level<select className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" value={form.level} onChange={(e) => setForm((prev) => ({ ...prev, level: e.target.value }))}>{['100','200','300','400','500'].map((level) => <option key={level} value={level}>{level}</option>)}</select></label>
